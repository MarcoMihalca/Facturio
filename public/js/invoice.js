...
function addProductRow(data = {}) {
    ...
    tr.innerHTML = `
        ...
        <td>
            <div class="tva-cell-wrap">
                <select name="tvaPercentSelect" onchange="window.handleTvaChange(this)">
                    <option value="21" ${data.tvaPercent === 21 || data.tvaPercent === undefined ? 'selected' : ''}>21%</option>
                    <option value="19" ${data.tvaPercent === 19 ? 'selected' : ''}>19%</option>
                    ...
                    <option value="custom" ${data.tvaPercent !== undefined && ![0, 5, 9, 19, 21].includes(parseFloat(data.tvaPercent)) ? 'selected' : ''}>Custom</option>
                </select>
                <input type="number" name="tvaPercentCustom" min="0" step="0.1" placeholder="%" 
                    style="display: ${data.tvaPercent !== undefined && ![0, 5, 9, 19, 21].includes(parseFloat(data.tvaPercent)) ? 'block' : 'none'}; width: 60px;" 
                    value="${![0, 5, 9, 19, 21].includes(parseFloat(data.tvaPercent)) ? data.tvaPercent : ''}">
            </div>
        </td>
